...
# --- FIXED: CLOUDFLARE AUTOMATION WITH QR CODE ---
...
def get_resource_path(relative_path):
    """ EXE ke andar ka temporary path dhoondne ke liye """
    try:
...
def start_cloudflare():
    # Ab hum get_resource_path use karenge cloudflared dhoondne ke liye
    ...
    try:
        ...
        # Baqi logic (line searching aur QR printing) bilkul same rahega
        for line in process.stdout:
            if "trycloudflare.com" in line:
                public_url = re.search(r'https://[a-zA-Z0-9-]+\.trycloudflare\.com', line)
                if public_url:
    ...
    def do_POST(self):
        ...
        if 'file' in form:
            ...
            for item in files:
                if item.filename:
                    target_dir = ROOT_DIR
                    if os.path.exists(os.path.join(ROOT_DIR, item.filename)):
                        target_dir = os.path.join(ROOT_DIR, "Duplicates")
                        if not os.path.exists(target_dir): os.makedirs(target_dir)
                    with open(os.path.join(target_dir, item.filename), 'wb') as f: f.write(item.file.read())
    ...
    def get_ui(self, current_rel_path, files):
        ...
        <head>
            ...
            <style>
                ...
                .item {{ background: white; margin-bottom: 10px; border-radius: 12px; border: 1px solid #e8eaed; display: flex; align-items: center; padding: 12px 10px; min-width: 0; transition: all 0.2s ease; animation: fadeInUp 0.3s ease backwards; }}
                .item:hover {{ cursor: pointer; transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.1); border-color: #1a73e8; }}
